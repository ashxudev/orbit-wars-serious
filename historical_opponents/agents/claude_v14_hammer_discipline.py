...
def think(obs, cfg):
        ...
        if funnel:
            ...
            if snipe_req > n:
                if snipe_req <= avail:
                    ...
                    contested = True
            # doomstack: ride the whole garrison (minus a small floor) on the
            # mission — surplus becomes the new garrison, the big fleet flies
            # faster, and the stack re-launches onward next turn
            floor = max(4, pprod[si] * 2)
            full = avail - floor
            if full > n:
                n = full
            if n > avail:
    ...
    # rally to the army: idle garrisons reinforce the largest stack so it
    # can keep rolling (elite play concentrates, then chains captures)
    if remaining > 40 and len(safe_my) >= 2:
        army = max(safe_my, key=lambda j: cur_ships[j])
        for si in safe_my:
            if si == army:
                continue
            n = cur_ships[si] - max(6, pprod[si] * 3)
            if n < 12:
                continue
            r = eval_mission(si, army, n_hint=n, funnel=True)
            if r is not None:
                V, n2, ang, T = r
                V += POS_BONUS * n2 * 2.0
                cands.append([V, si, army, n2, ang, T, "funnel"])

    # ---- greedy commit loop ----
    # concentration discipline (elite pattern): at most ONE offensive
    # launch per turn — garrisons accumulate between hammer blows;
    # defense, evacuation and rally are housekeeping and stay unlimited
    moves = []
    committed_ship_count = {}
    offensive_done = False
    for _round in range(MAX_MOVES):
        ...
        if offensive_done:
            cands = [c for c in cands
                     if c[6] in ("evac", "funnel") or owner[c[2]] == me]
            if not cands:
                break
        best = cands[0]
        ...
        if kind not in ("evac", "funnel", "relay"):
            ...
            n, ang, T = n2, ang2, T2
        # commit
        if kind not in ("evac", "funnel") and owner[ti] != me:
            offensive_done = True
